...
def clean_location(location_string: str) -> str:
    ...
    if isinstance(location_string, str):
        ...
        main_location = (
            location_string.replace("SUBCELLULAR LOCATION: ", "")
            .split(".")[0]
            .split(" {")[0]
            .strip()
        )
...
def group_locations_hierarchical(label: str) -> str:
    ...
    if (
        "cytoplasm" in label_lower
        or "cytosol" in label_lower
        or "cytoskeleton" in label_lower
    ):
        return "Cytoplasm"
...
def process_data(input_path: str, output_path: str, min_samples: int):
    ...
    # 1. Limpieza inicial
    df_processed["location_label"] = df_processed["subcellular_location"].apply(
        clean_location
    )
    ...
    df_processed["location_label"] = df_processed["location_label"].apply(
        group_locations_hierarchical
    )

    # 3. Excluimos las clases 'Other' y 'Unknown'
    df_processed = df_processed[
        ~df_processed["location_label"].isin(["Other", "Unknown"])
    ]
    ...
    print(
        f"Datos procesados y guardados en {output_path}. Total de filas: {
            len(df_final)
        }"
    )
    return df_final
